...
const usage = (tx: Tx, keys: string[]) => tx`
  select distinct lower(v) as word, c.id as card_id, null::uuid as devotion_id
  ...
  union
  select k.word::text, m.card_id, null::uuid
  from keyword_marks m join keywords k on k.id = m.keyword_id join cards c on c.id = m.card_id and c.status = 'saved'
  union
  select distinct lower(v), null::uuid, d.id from devotions d, unnest(d.subjects) v
  union
  select k.word::text, null::uuid, m.devotion_id
  from keyword_marks m join keywords k on k.id = m.keyword_id where m.devotion_id is not null`
...
keywords.get('/keywords', async (c) => {
  const rows = await withUser(c.get('userId'), async (tx) => tx`
    select k.word, count(u.word)::int as count,
...
keywords.get('/keywords/:word', async (c) => {
  ...
  const result = await withUser(c.get('userId'), async (tx) => {
    ...
    const devotions = await tx`
      select d.id, d.date::text as date, d.scripture, d.content,
             coalesce(json_agg(json_build_object('id', m.id, 'start', m.start_offset, 'end', m.end_offset, 'is_definition', m.is_definition)
                               order by m.start_offset) filter (where m.id is not null), '[]') as marks
      from (${usage(tx, await tagKeys(tx))}) u
      join devotions d on d.id = u.devotion_id
      left join keywords k on k.word = ${word}
      left join keyword_marks m on m.devotion_id = d.id and m.keyword_id = k.id
      where u.word = ${word}
    ...
    return {
      devotions: devotions.map((d) => ({
        id: d.id, date: d.date, scripture: d.scripture,
        excerpts: (d.marks as { id: string; start: number; end: number; is_definition: boolean }[]).map((m) => ({
          mark_id: m.id, is_definition: m.is_definition, text: excerpt(d.content ?? '', m.start, m.end),
        })),
      })),
      strongs,
...
export const Marks = z.array(z.strictObject({ start: z.number().int().min(0), end: z.number().int().min(1), is_definition: z.boolean() })).max(500)

type MarkOwner = { card_id: string } | { devotion_id: string }
const ownedBy = (tx: Tx, owner: MarkOwner) => ('card_id' in owner ? tx`card_id = ${owner.card_id}` : tx`devotion_id = ${owner.devotion_id}`)

export const listMarks = (tx: Tx, owner: MarkOwner) => tx`
  select id, start_offset as start, end_offset as end, label, is_definition from keyword_marks where ${ownedBy(tx, owner)} order by start_offset`

// Replaces the marks on one text. Labels are read from that text here, never trusted from the client.
export async function replaceMarks(tx: Tx, owner: MarkOwner, text: string, marks: z.infer<typeof Marks>) {
  ...
  if (wanted.some((m) => m.end <= m.start || m.end > text.length || !m.label.trim() || m.label.includes('\n') || m.label.length > 100))
    fail(400, 'a mark does not fit the text')
  await tx`delete from keyword_marks where ${ownedBy(tx, owner)}`
  for (const m of wanted) {
    const [word] = await tx`
      insert into keywords (word) values (${m.label.trim().toLowerCase()})
      on conflict (user_id, word) do update set word = excluded.word returning id`
    await tx`insert into keyword_marks ${tx({ ...owner, keyword_id: word.id, start_offset: m.start, end_offset: m.end, label: m.label, is_definition: m.is_definition })}`
  }
  await pruneOrphans(tx)
  return listMarks(tx, owner)
}
...
keywords.put('/cards/:id/marks', async (c) => {
  ...
  const rows = await withUser(c.get('userId'), async (tx) => {
    ...
    return replaceMarks(tx, { card_id: id }, card.transcription ?? '', marks)
  })
  return c.json(rows)
})
